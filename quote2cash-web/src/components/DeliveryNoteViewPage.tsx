...
export default function DeliveryNoteViewPage({ deliveryNote, onEdit, onBack }: Props) {
  ...
  const handleDownloadPdf = async () => {
    try {
      const fullDeliveryNote = await getDeliveryNote(deliveryNote.id);
      generateDeliveryNotePDF(fullDeliveryNote);
    } catch (error) {
      console.error('Error fetching delivery note data for PDF:', error);
      alert('Failed to fetch delivery note data for PDF generation. Please try again.');
    }
  };
    ...
    <div className="page-section">
      <style dangerouslySetInnerHTML={{
        ...
        @media print {
          ...
          .dn-items-table-header, .dn-items-table-row { display: grid !important; grid-template-columns: 80px 1fr !important; }
      ...
      <div className="view-container">
        <div className="view-card" style={{ color: '#000' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '24px', marginBottom: '18px' }}>
            ...
            {/* Right: DELIVERY NOTE details + client box */}
            <div style={{ textAlign: 'right', fontSize: '0.75rem', lineHeight: '1.4' }}>
              <div style={{ fontSize: '0.9rem', fontWeight: 'bold', marginBottom: '6px', letterSpacing: '1px' }}>DELIVERY NOTE</div>

              <div style={{ fontSize: '0.75rem', marginBottom: '2px' }}>
                <span style={{ fontWeight: 600 }}>DELIVERY NOTE NUMBER: </span>{deliveryNote.deliveryNoteNumber}
              </div>
              <div style={{ fontSize: '0.75rem', marginBottom: '2px' }}>
                <span style={{ fontWeight: 600 }}>DATE: </span>{createdDate}
              </div>
              <div style={{ fontSize: '0.75rem', marginBottom: '2px' }}>
                <span style={{ fontWeight: 600 }}>REFERENCE: </span>{deliveryNote.reference || '—'}
              </div>
              <div style={{ fontSize: '0.75rem', marginBottom: '2px' }}>
          ...
          <div style={{ marginTop: '20px', border: '1px solid #000', borderRadius: '2px', overflow: 'hidden' }}>
            {/* Header */}
            <div className="dn-items-table-header" style={{ display: 'grid', gridTemplateColumns: '80px 80px 1fr', background: '#f3f4f6', borderBottom: '1px solid #000' }}>
            ...
            {items.length > 0 ? (
              items.map((item) => (
                <div key={item.id} className="dn-items-table-row" style={{ display: 'grid', gridTemplateColumns: '80px 80px 1fr', borderTop: '1px solid #eee' }}>
          ...
          {deliveryNote.description && (
            <div style={{ marginTop: '16px', fontSize: '0.8rem', lineHeight: '1.5' }}>
              <strong>REMARKS:</strong> {deliveryNote.description}
            </div>
          )}
