...
function saveNestedItems(PDO $pdo, string $table, string $foreignKey, string $foreignId, array $items): void
{
    foreach ($items as $item) {
        if ($table === 'quoteitems' && trim((string)($item['code'] ?? '')) !== '') { $q = $pdo->prepare('SELECT * FROM products WHERE ' . dbColumn('code') . ' = ?'); $q->execute([trim((string)$item['code'])]); $product = databaseRow($q->fetch(PDO::FETCH_ASSOC) ?: []); if (!$product) { $product = ['id'=>uuid(),'code'=>trim((string)$item['code']),'name'=>trim((string)($item['description'] ?? $item['code'])),'uom'=>(string)($item['uom'] ?? ''),'description'=>$item['description'] ?? null,'price'=>(float)($item['unitPrice'] ?? 0),'image'=>$item['imagePath'] ?? null]; $columns = array_map('dbColumn', array_keys($product)); $pdo->prepare('INSERT INTO products (' . implode(',', $columns) . ') VALUES (?,?,?,?,?,?,?)')->execute(array_values($product)); } $item['uom'] = $item['uom'] ?? $product['uom']; $item['description'] = $item['description'] ?? $product['description']; $item['unitPrice'] = (float)($item['unitPrice'] ?? 0) ?: (float)$product['price']; $item['imagePath'] = $item['imagePath'] ?? $product['image']; $item['productId'] = $product['id']; }
